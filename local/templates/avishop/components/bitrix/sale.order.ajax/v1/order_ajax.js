		...
		BX.Sale &&
			...
			(BX.Sale.Input.Utils.asMultiple = function (value) {
				...
				if (value.constructor === Array) {
					for (var i = 0, length = value.length, val; i < length;)
						null == (val = value[i]) || "" === val ? (value.splice(i, 1), --length) : ++i;
			...
			(BX.Sale.OrderAjaxComponent = {
					...
					this.initializePrimaryFields(),
						...
						((this.authBlockNode.style.display = ""),
							...
							(this.authGenerateUser =
								"Y" !== this.result.AUTH.new_user_registration_email_confirmation &&
						...
						(this.result.IS_AUTHORIZED && void 0 === this.result.LAST_ORDER_DATA.FAIL) ||
						...
						"Y" === this.params.USE_ENHANCED_ECOMMERCE && this.setAnalyticsDataLayer("checkout"),
				...
				sendRequest: function (action, actionData) {
					...
					this.startLoader() &&
						((this.firstLoad = !1),
							"saveOrderAjax" === (action = BX.type.isNotEmptyString(action) ? action : "refreshOrderAjax")
								...
								: BX.ajax({
									...
									onsuccess: BX.delegate(function (result) {
										...
										(result.redirect &&
											...
											case "enterCoupon":
												result && result.order
													? ((this.deliveryCachedInfo = []), this.refreshOrder(result))
				...
				getData: function (action, actionData) {
					...
					return (
						...
						("enterCoupon" !== action && "removeCoupon" !== action) || (data.coupon = actionData),
						data
					);
				},
						...
						if (
							(this.isPriceChanged(result),
								this.isDeliveryChanged
									? (this.isDeliveryChanged = !1)
									: this.activeSectionId !== this.deliveryBlockNode.id && (this.deliveryCachedInfo = []),
								result.order.PAY_CURRENT_ACCOUNT && "Y" === result.order.PAY_CURRENT_ACCOUNT)
						) {
				...
				endLoader: function () {
					(this.BXFormPosting = !1),
						this.loadingScreen && this.loadingScreen.isShown() && this.loadingScreen.close();
				},
				...
				getFileAccepts: function (accepts) {
					...
					for (i = 0; i < arAccepts.length; i++)
						(currentAccept = mimeTypesMap[(currentAccept = BX.util.trim(arAccepts[i]))] || currentAccept),
				...
				showBlockErrors: function (node) {
					...
					if (errorNode) {
						...
						errors &&
							...
							(this.showError(node, errors, !0), this.showError(hiddenNode, errors));
					}
				},
				checkNotifications: function () {
					var informer = this.mainErrorsNode.querySelector('[data-type="informer"]'),
						...
						informerPos;
					informer &&
						(this.firstLoad && this.result.IS_AUTHORIZED && void 0 === this.result.LAST_ORDER_DATA.FAIL
							? ((className = (success =
								(sections = this.orderBlockNode.querySelectorAll(".bx-soa-section.bx-active"))
									.length && "true" == sections[sections.length - 1].getAttribute("data-visited"))
								? "success"
								: "warning"),
								(text = (
									success ? this.params.MESS_SUCCESS_PRELOAD_TEXT : this.params.MESS_FAIL_PRELOAD_TEXT
								)
									.split("#ORDER_BUTTON#")
							...
							((scrollTop = BX.GetWindowScrollPos().scrollTop),
								...
								new BX.easing({
									...
									complete: function () {
										scrollTop > informerPos.top && window.scrollBy(0, -(informerPos.height + 20)),
					...
					if (
						(currentDelivery && currentDelivery.CALCULATE_ERRORS
							? (BX.addClass(this.deliveryBlockNode, "bx-step-warning"),
								...
								(warningString +=
									"<br><small>" + this.params.MESS_DELIVERY_CALC_ERROR_TEXT + "</small>"),
						...
						for (k in this.result.WARNING)
							if (this.result.WARNING.hasOwnProperty(k))
								switch (k.toUpperCase()) {
									case "DELIVERY":
										"true" === this.deliveryBlockNode.getAttribute("data-visited") &&
											(this.showBlockWarning(this.deliveryBlockNode, this.result.WARNING[k], !0),
										...
										break;
									case "PAY_SYSTEM":
										"true" === this.paySystemBlockNode.getAttribute("data-visited") &&
											(this.showBlockWarning(this.paySystemBlockNode, this.result.WARNING[k], !0),
												this.showBlockWarning(
				...
				showBlockWarning: function (node, warnings, hide) {
					...
					if (errorNode) {
						if (BX.type.isString(warnings)) warnStr = warnings;
							...
							for (i in warnings)
								warnings.hasOwnProperty(i) && warnings[i] && (warnStr += warnings[i] + "<br>");
						if (!warnStr) return;
						for (i in (existedWarningNodes = node.querySelectorAll(".alert.alert-warning")))
							if (
					...
					if (node && entity) {
						...
						if (pagination.pages.length > 1) {
							for (
								...
								(active = (pageCounter = parseInt(i) + 1) == pagination.pageNumber ? "bx-active" : ""),
									navigation.push(
										BX.create("LI", {
											attrs: { "data-action": pageCounter, "data-entity": entity },
				...
				doPagination: function (e) {
					...
					return BX.hasClass(node, "bx-active")
						...
						: (("prev" != page && "next" != page) ||
							((pageNum = parseInt(
								BX.findParent(node).querySelector(".bx-active").getAttribute("data-action")
							)),
					...
					for (
						...
						((hidden = this.deliveryBlockNode.querySelector("input[type=hidden][name=DELIVERY_ID]")) ||
							(hidden = BX.create("INPUT", {
								props: { type: "hidden", name: "DELIVERY_ID", value: selectedDelivery.ID },
							}))),
						deliveryItemsContainer = this.deliveryBlockNode.querySelector(".bx-soa-pp-item-container"),
						BX.cleanNode(deliveryItemsContainer),
						BX.type.isDomNode(hidden) && BX.prepend(hidden, BX.findParent(deliveryItemsContainer)),
						k = 0;
				...
				showPaySystemItemsPage: function (page) {
					...
					for (
						...
						((hidden = this.paySystemBlockNode.querySelector(
							...
							(hidden = BX.create("INPUT", {
								props: { type: "hidden", name: "PAY_SYSTEM_ID", value: selectedPaySystem.ID },
							}))),
						paySystemItemsContainer =
						this.paySystemBlockNode.querySelector(".bx-soa-pp-item-container"),
						BX.cleanNode(paySystemItemsContainer),
						BX.type.isDomNode(hidden) && BX.prepend(hidden, BX.findParent(paySystemItemsContainer)),
						k = 0;
				...
				initPropsListForLocation: function () {
					...
					if (BX.saleOrderAjax && this.result.ORDER_PROP && this.result.ORDER_PROP.properties)
						for (BX.saleOrderAjax.cleanUp(), i = 0; i < this.result.ORDER_PROP.properties.length; i++)
							if (
								"LOCATION" == (curProp = this.result.ORDER_PROP.properties[i]).TYPE &&
								"Y" == curProp.MULTIPLE &&
								...
								(attrObj = {
									...
									"Y" != curProp.IS_ZIP ||
									((attrObj.isZip = !0), (this.deliveryLocationInfo.zip = curProp.ID)),
									BX.saleOrderAjax.addPropertyDesc({ id: curProp.ID, attributes: attrObj });
				},
				bindEvents: function () {
					BX.bind(
				...
				initFirstSection: function () {
					...
					firstSection.id != this.basketBlockNode.id &&
						(BX.addClass(firstSection, "bx-selected"), (this.activeSectionId = firstSection.id));
				},
				initOptions: function () {
					...
					if (
						...
						for (headers = this.result.GRID.HEADERS, i = 0; i < headers.length; i++)
							"PREVIEW_PICTURE" === headers[i].id && (this.options.showPreviewPicInBasket = !0),
								"DETAIL_PICTURE" === headers[i].id && (this.options.showDetailPicInBasket = !0),
								"PROPS" === headers[i].id && (this.options.showPropsInBasket = !0),
								"NOTES" === headers[i].id && (this.options.showPriceNotesInBasket = !0);
					this.result.TOTAL &&
				...
				reachGoal: function (goal, section) {
					...
					"Y" == this.params.USE_YM_GOALS &&
						...
						((goalId = this.getGoalId(goal, section)), window["yaCounter" + counter].reachGoal(goalId));
				},
				...
				initPagination: function () {
					...
					if (this.result.DELIVERY)
						if (
							...
							for (this.deliveryPagination.pages = arPages, i = 0; i < this.result.DELIVERY.length; i++)
				...
				initPickUpPagination: function () {
					...
					if (this.options.pickUpsPerPage >= 0 && this.result.DELIVERY)
						for (i = 0; i < this.result.DELIVERY.length; i++)
							if ("Y" === this.result.DELIVERY[i].CHECKED && this.result.DELIVERY[i].STORE_MAIN) {
								(usePickUp = this.result.DELIVERY[i].STORE_MAIN.length > 0),
									(usePickUpPagination =
										this.result.DELIVERY[i].STORE_MAIN.length > this.options.pickUpsPerPage),
									usePickUp && (stores = this.getPickUpInfoArray(this.result.DELIVERY[i].STORE_MAIN));
								break;
							}
					if (usePickUp)
						if (this.options.pickUpsPerPage > 0 && usePickUpPagination) {
							...
							for (this.pickUpPagination.pages = arPages, i = 0; i < stores.length; i++)
							...
							this.pickUpPagination.pageNumber || (this.pickUpPagination.pageNumber = 1),
				...
				prepareLocations: function (locations) {
					...
					if (((this.locations = {}), (this.cleanLocations = {}), BX.util.object_keys(locations).length))
						for (i in locations)
							if (locations.hasOwnProperty(i)) {
								for (k in ((this.locationsTemplate = locations[i].template || ""),
									(temporaryLocations = []),
									(output = locations[i].output).clean &&
									((this.cleanLocations[i] = BX.processHTML(output.clean, !1)), delete output.clean),
									output))
							...
							((clearButton = locationNode.querySelector("div.bx-ui-sls-clear")),
								...
								BX.bind(clearButton, "click", function (e) {
									var target = e.target || e.srcElement,
										...
										locationInput;
									parent && (locationInput = parent.querySelector("input.bx-ui-sls-fake[type=text]")),
						...
						if (allSections[i].id === actionSection.id && allSections[i + 1])
							return (
								...
								this.shouldSkipSection(nextSection)
									...
									: { prev: actionSection, next: nextSection, skip: skippedSection }
							);
						...
						if (allSections[i].id === actionSection.id && allSections[i - 1])
							return (
								...
								this.shouldSkipSection(prevSection)
									? (this.markSectionAsCompleted(prevSection), this.getPrevSection(prevSection))
					...
					if (node && node.id && this.activeSectionId == node.id) {
						...
						if (
							...
							var windowScrollTop = BX.GetWindowScrollPos().scrollTop,
								...
								nextSectionHidden,
								offset;
							((nextSectionHidden = BX(nextSection.id + "-hidden")).style.left = "-10000"),
				...
				showActualBlock: function () {
					for (
						var allSections = this.orderBlockNode.querySelectorAll(".bx-soa-section.bx-active"), i = 0;
						allSections[i];

					) {
						if (
							(allSections[i].id === this.regionBlockNode.id && this.isValidRegionBlock(),
								allSections[i].id === this.propsBlockNode.id && this.isValidPropertiesBlock(),
								!this.checkBlockErrors(allSections[i]) || !this.checkPreload(allSections[i]))
						) {
				...
				getBlockFooter: function (node) {
					...
					currentSection.id == this.basketBlockNode.id &&
						(currentSection && currentSection.id.indexOf(firstSection.id),
							currentSection && "-1" != currentSection.id.indexOf(lastSection.id) && (isLastNode = !0));
				},
				...
				switchOrderSaveButtons: function (state) {
					...
					("" == this.orderSaveBlockNode.style.display) != state &&
						...
						((orderSaveNode.style.opacity = 0),
							...
							totalButton && ((totalButton.style.opacity = 0), (totalButton.style.display = "")),
							mobileButton && ((mobileButton.style.opacity = 0), (mobileButton.style.display = "")),
							new BX.easing({
								duration: 500,
								start: { opacity: 0 },
								finish: { opacity: 100 },
								...
								step: function (state) {
									(orderSaveNode.style.opacity = state.opacity / 100),
										totalButton && (totalButton.style.opacity = state.opacity / 100),
										mobileButton && (mobileButton.style.opacity = state.opacity / 100);
								},
								complete: function () {
				...
				changeVisibleContent: function () {
					...
					for (i = 0; i < sections.length; i++)
						(state =
							(state = this.firstLoad && orderDataLoaded) || this.shouldBeSectionVisible(sections, i)),
				...
				editSection: function (section) {
					if (section && section.id) {
						this.result.SHOW_AUTH &&
							...
							: section.id != this.pickUpBlockNode.id && (section.style.display = "");
				...
				editAuthBlock: function () {
					if (this.authBlockNode) {
						...
						BX.hasClass(authContent, "reg")
							? ((regContent = authContent), (authContent = BX.firstChild(this.authHiddenBlockNode)))
				...
				editAuthorizeForm: function (authContent) {
					...
					(login = this.createAuthFormInputContainer(
								...
								BX.create("DIV", {
									...
									children: [
										BX.create("LABEL", {
											...
											children: [
												BX.create("INPUT", {
													props: { type: "checkbox", name: "USER_REMEMBER", value: "Y" },
												}),
												BX.create("SPAN", {
						...
						(button = BX.create("DIV", {
							...
							children: [
								BX.create("INPUT", {
									props: { id: "do_authorize", type: "hidden", name: "do_authorize", value: "N" },
								}),
								BX.create("button", {
						...
						authContent.appendChild(
							BX.create("DIV", { props: { className: "col-md-6" }, children: [authFormNode] })
						);
				},
				createAuthFormInputContainer: function (labelText, inputNode, required) {
					var labelHtml = "";
					return (
						required && (labelHtml += '<span class="bx-authform-starrequired">*</span>'),
				...
				editRegistrationForm: function (authContent) {
					if (this.result.AUTH) {
						var authFormNodes = [],
							showSmsConfirm = this.result.SMS_AUTH && "OK" === this.result.SMS_AUTH.TYPE;
						showSmsConfirm
							? (authFormNodes.push(
								BX.create("DIV", {
									props: { className: "alert alert-success" },
									...
									BX.create("LABEL", {
										...
										events: {
											change: BX.delegate(function () {
												var generated;
												(this.authBlockNode.querySelector(".generated").style.display = ""),
									...
									BX.create("DIV", {
										...
										children: [
											...
											this.createAuthFormInputContainer(
												BX.message("STOF_PASSWORD"),
												BX.create("INPUT", {
													props: { name: "NEW_PASSWORD", type: "password", size: 30 },
													events: { keypress: BX.proxy(this.checkKeyPress, this) },
												}),
												!0
											),
											this.createAuthFormInputContainer(
												...
												BX.create("INPUT", {
													props: { name: "NEW_PASSWORD_CONFIRM", type: "password", size: 30 },
													events: { keypress: BX.proxy(this.checkKeyPress, this) },
												}),
												!0
											),
				...
				editSocialContent: function (authContent) {
					if (BX("bx-soa-soc-auth-services")) {
						...
						if (!1 === this.socServiceHiddenNode) {
							var socServiceHiddenNode =
								BX("bx-soa-soc-auth-services").querySelector(".bx-authform-social");
				...
				alignBasketColumns: function () {
					if (this.basketBlockNode) {
						...
						if (windowSize.innerWidth > 580 && windowSize.innerWidth < 992) {
							if (
								((percent = 100),
									(basketRows = this.basketBlockNode.querySelectorAll(".bx-soa-basket-info")).length)
							) {
								if (
									(columnNodes = basketRows[0].querySelectorAll(".bx-soa-item-properties")).length &&
									"" != columnNodes[0].style.width
								)
									return;
								if ((columns = columnNodes.length) > 0)
									for (
										columns = columns > 4 ? 4 : columns, percent = parseInt(percent / columns);
										i < basketRows.length;
										i++
									)
										for (
							...
							if (
								...
								return;
							for (; i < columnNodes.length; i++) columnNodes[i].style.width = "";
						}
					}
				},
					...
					this.initialized.basket
						...
						: ((basketContent = node.querySelector(".bx-soa-section-content")),
							...
							basketContent
								...
								: ((basketContent = this.getNewContainer()), node.appendChild(basketContent)),
							...
							"Y" === this.params.SHOW_COUPONS_BASKET && this.editCoupons(basketContent),
				...
				editFadeBasketBlock: function (activeNodeMode) {
					...
					this.initialized.basket ||
						((newContent = this.getNewContainer()),
							...
							"Y" === this.params.SHOW_COUPONS_BASKET && this.editCoupons(newContent),
					...
					if (basketItemsNode) {
						...
						for (i = 0; i < this.result.GRID.HEADERS.length; i++)
							"NAME" !== (column = this.result.GRID.HEADERS[i]).id &&
								...
								"PROPS" !== column.id &&
								"NOTES" !== column.id &&
								("DETAIL_PICTURE" !== column.id || this.options.showPreviewPicInBasket) &&
				...
				createBasketItem: function (basketItemsNode, item, index, active) {
					var mainColumns = [],
						...
						countCols = 0;
					for (
						(this.options.showPreviewPicInBasket || this.options.showDetailPicInBasket) &&
						...
						"NAME" !== (currentColumn = this.result.GRID.HEADERS[i]).id &&
							...
							("DETAIL_PICTURE" !== currentColumn.id || this.options.showPreviewPicInBasket) &&
					...
					cols = [
						BX.create("TD", {
							...
							children: [
								BX.create("DIV", { props: { className: "bx-soa-item-block" }, children: mainColumns }),
							],
						}),
					].concat(otherColumns);
					var dopClassServices = "",
						withServices =
							void 0 !== item.data.LINK_SERVICES && item.data.LINK_SERVICES.constructor === Object;
					"Y" === item.data.IS_SERVICES && (dopClassServices += " hidden_order_services "),
					...
					(countCols = dopColCount + countCols + 1),
						withServices &&
						((servicesItems = this.createServicesBasketItem(item, countCols)).forEach(function (
							elem,
							i,
							arr
						) {
						...
						basketItemsNode.appendChild(
							BX.create("TR", {
								props: {
									className: "bx-soa-item-tr bx-soa-item-info-container" + dopClassServices,
								},
								children: [
									BX.create("TD", {
										props: { className: "bx-soa-item-td" },
					...
					if (column && allData) {
						...
						if ("PRICE_FORMATED" === column.id)
							textNode.appendChild(
								BX.create("STRONG", { props: { className: "bx-price" }, html: data.PRICE_FORMATED })
							),
							...
							textNode.appendChild(
								BX.create("STRONG", { props: { className: "bx-price all" }, html: data.SUM })
							),
								parseFloat(data.DISCOUNT_PRICE) > 0 &&
								(textNode.appendChild(BX.create("BR")),
									textNode.appendChild(
								...
								for (var i in columnData)
									columnData.hasOwnProperty(i) &&
										("image" == columnData[i].type
											? val.push(
												this.getImageContainer(columnData[i].value, columnData[i].source)
											)
					...
					if (column && allData) {
						...
						if ("PROPS" !== column.id) {
							if ("PRICE_FORMATED" === column.id)
								...
								textNode.appendChild(
									BX.create("STRONG", { props: { className: "bx-price all" }, text: data.SUM })
								);
							else if ("DISCOUNT" === column.id)
								textNode.appendChild(
									BX.create("STRONG", {
							...
							else if ("DETAIL_PICTURE" === column.id || "PREVIEW_PICTURE" === column.id)
								(logotype = this.getImageSources(allData.data, column.id)),
									(img = BX.create("IMG", {
										props: { src: (logotype && logotype.src_1x) || this.defaultBasketItemLogo },
										style: { maxWidth: "50%" },
						...
						couponsBlock = BX.create("DIV", {
							...
							children: [
								BX.create("DIV", {
									...
									children: [
										BX.create("INPUT", {
											props: { className: "form-control bx-ios-fix", type: "text" },
											events: {
				...
				getCouponsList: function (active) {
					...
					for (i = 0; i < this.result.COUPON_LIST.length; i++)
						(active || (!active && "APPLIED" == this.result.COUPON_LIST[i].JS_STATUS)) &&
							couponsList.push(
								this.getCouponNode(
									{
				...
				getCouponNode: function (coupon, active) {
					...
					return BX.create("STRONG", {
						attrs: { "data-coupon": couponName, className: "bx-soa-coupon-item-" + couponItem },
						children: active
							? [
								couponName || "",
								BX.create("SPAN", {
				...
				addCoupon: function (coupon) {
					for (
						...
						!couponListNodes[i].querySelector('[data-coupon="' + BX.util.htmlspecialchars(coupon) + '"]');
						i++
					)
						couponListNodes[i].appendChild(
							this.getCouponNode({ text: coupon }, !0, "bx-soa-coupon-item-danger")
						);
				},
				...
				editRegionBlock: function (active) {
					this.regionBlockNode &&
						...
						(active
							? (this.editActiveRegionBlock(!0), !this.regionBlockNotEmpty && this.editFadeRegionBlock())
							: this.editFadeRegionBlock(),
				...
				editActiveRegionBlock: function (activeNodeMode) {
					...
					this.initialized.region ||
						((regionContent = node.querySelector(".bx-soa-section-content"))
							...
							: ((regionContent = this.getNewContainer()), node.appendChild(regionContent)),
				...
				editFadeRegionBlock: function () {
					var regionContent = this.regionBlockNode.querySelector(".bx-soa-section-content"),
						newContent;
					this.initialized.region ||
						(this.editActiveRegionBlock(!1), BX.remove(BX.lastChild(this.regionBlockNode))),
				...
				getSelectedPersonType: function () {
					...
					if (
						...
						for (i in ((personTypeId = personTypeInput.value), this.result.PERSON_TYPE))
				...
				getDeliveryLocationInput: function (node) {
					...
					for (k in this.result.ORDER_PROP.properties)
						if (
							...
							"Y" == (currentProperty = this.result.ORDER_PROP.properties[k]).IS_LOCATION
						) {
					...
					if ((location = this.locations[locationId]) && location[0] && location[0].output)
						for (k in ((this.regionBlockNotEmpty = !0),
							...
							node.appendChild(
								BX.create("INPUT", {
									props: { type: "hidden", name: "RECENT_DELIVERY_VALUE", value: location[0].lastValue },
								})
							),
					...
					altProperty &&
						((altNode = BX.create("DIV", {
							...
							(labelTextHtml =
								"Y" == altProperty.REQUIRED ? '<span class="bx-authform-starrequired">*</span> ' : ""),
				...
				getZipLocationInput: function (node) {
					...
					zipProperty &&
						((this.regionBlockNotEmpty = !0),
							...
							(labelTextHtml =
								"Y" == zipProperty.REQUIRED ? '<span class="bx-authform-starrequired">*</span> ' : ""),
				...
				getPersonTypeControl: function (node) {
					if (this.result.PERSON_TYPE) {
							...
							for (i in this.result.PERSON_TYPE)
								this.result.PERSON_TYPE.hasOwnProperty(i) &&
									((currentType = this.result.PERSON_TYPE[i]),
										options.push(
											BX.create("OPTION", {
												props: { value: currentType.ID, selected: "Y" == currentType.CHECKED },
							...
							for (i in this.result.PERSON_TYPE)
								this.result.PERSON_TYPE.hasOwnProperty(i) &&
									((currentType = this.result.PERSON_TYPE[i]),
										...
										node.appendChild(
											BX.create("DIV", { props: { className: "radio-inline" }, children: [label] })
										),
										(delimiter = !0),
										"Y" == currentType.CHECKED && (oldPersonTypeId = currentType.ID));
							this.regionBlockNotEmpty = !0;
						...
						oldPersonTypeId &&
							node.appendChild(
								BX.create("INPUT", {
									props: { type: "hidden", name: "PERSON_TYPE_OLD", value: oldPersonTypeId },
								})
							);
					}
				},
						...
						if (
							...
							"Y" === this.params.ALLOW_NEW_PROFILE &&
								...
								(profileChangeInput = BX.create("INPUT", {
									props: { type: "hidden", value: "N", id: "profile_change", name: "profile_change" },
								})),
				...
				editPaySystemBlock: function (active) {
					this.paySystemBlockNode &&
						this.paySystemHiddenBlockNode &&
						this.result.PAY_SYSTEM &&
						(active ? this.editActivePaySystemBlock(!0) : this.editFadePaySystemBlock(),
					...
					if (this.initialized.paySystem);
						...
						if (
							((paySystemContent = node.querySelector(".bx-soa-section-content"))
								...
								: ((paySystemContent = this.getNewContainer()), node.appendChild(paySystemContent)),
								...
								(this.editCoupons(paySystemContent), !this.couponsCustomInitialized))
						) {
				...
				editFadePaySystemBlock: function () {
					...
					this.initialized.paySystem ||
						(this.editActivePaySystemBlock(!1), BX.remove(BX.lastChild(this.paySystemBlockNode))),
						(newContent = this.getNewContainer(!0)),
						this.paySystemBlockNode.appendChild(newContent),
						this.editFadePaySystemContent(newContent),
						"Y" == this.params.SHOW_COUPONS_PAY_SYSTEM && this.editCoupons(newContent);
				},
				editPaySystemItems: function (paySystemNode) {
					if (this.result.PAY_SYSTEM && !(this.result.PAY_SYSTEM.length <= 0)) {
						...
						this.paySystemPagination.show && this.showPagination("paySystem", paySystemItemsContainer),
				...
				editPaySystemInfo: function (paySystemNode) {
					if (
						this.result.PAY_SYSTEM &&
						(0 != this.result.PAY_SYSTEM.length || "Y" == this.result.PAY_FROM_ACCOUNT)
					) {
						var paySystemInfoContainer = BX.create("DIV", {
							props: {
								className:
									(0 == this.result.PAY_SYSTEM.length ? "col-sm-12" : "col-sm-5") +
						...
						if (injectNodes && injectNodes.length)
							for (var i = 0; i < injectNodes.length; ++i)
								BX.addClass(injectNodes[i], "hidden"), BX.cleanNode(injectNodes[i]);
						if (currentPaySystem)
							if (
				...
				getInnerPaySystem: function () {
					if (
						...
						var accountOnly =
							this.params.ONLY_FULL_PAY_FROM_ACCOUNT && "Y" == this.params.ONLY_FULL_PAY_FROM_ACCOUNT,
				...
				editFadePaySystemContent: function (node) {
					var selectedPaySystem = this.getSelectedPaySystem(),
						...
						logotype,
						imgSrc;
					errorNode ? node.appendChild(errorNode.cloneNode(!0)) : this.getErrorContainer(node),
						warningNode && warningNode.innerHTML && node.appendChild(warningNode.cloneNode(!0)),
				...
				getSelectedPaySystem: function () {
					...
					if (
						...
						for (paySystemId = paySystemCheckbox.value, i = 0; i < this.result.PAY_SYSTEM.length; i++)
					...
					if (this.orderBlockNode && event) {
						var target = event.target || event.srcElement,
							...
							fullPayFromInnerPaySystem =
								this.result.TOTAL && 0 === parseFloat(this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY),
						...
						if (innerPsAction)
							...
							innerPaySystemCheckbox && innerPaySystemCheckbox.checked && fullPayFromInnerPaySystem
								? (BX.addClass(actionSection, "bx-selected"),
									((actionInput = actionSection.querySelector("input[type=checkbox]")).checked = !0),
									BX.removeClass(innerPaySystemSection, "bx-selected"),
									(innerPaySystemCheckbox.checked = !1))
								: ((selectedSection = this.paySystemBlockNode.querySelector(
									...
									(BX.removeClass(selectedSection, "bx-selected"),
										(selectedSection.querySelector("input[type=checkbox]").checked = !1)));
						}
						this.sendRequest();
					}
				},
				editDeliveryBlock: function (active) {
					this.deliveryBlockNode &&
						this.deliveryHiddenBlockNode &&
						this.result.DELIVERY &&
						(active ? this.editActiveDeliveryBlock(!0) : this.editFadeDeliveryBlock(),
							this.checkPickUpShow(),
					...
					if (this.initialized.delivery);
						...
						if (
							((deliveryContent = node.querySelector(".bx-soa-section-content"))
								...
								: ((deliveryContent = this.getNewContainer()), node.appendChild(deliveryContent)),
								...
								(this.editCoupons(deliveryContent), !this.couponsCustomInitialized))
						) {
							var divs = BX.findChildren(BX("bx-soa-coupon"), { className: "bx-soa-section-content" });
							divs &&
								divs.length &&
				...
				editDeliveryItems: function (deliveryNode) {
					if (this.result.DELIVERY && !(this.result.DELIVERY.length <= 0)) {
						...
						for (k = 0; k < this.deliveryPagination.currentPage.length; k++)
							(deliveryItemNode = this.createDeliveryItem(this.deliveryPagination.currentPage[k])),
								deliveryItemsContainer.appendChild(deliveryItemNode);
						this.deliveryPagination.show && this.showPagination("delivery", deliveryItemsContainer),
				...
				editDeliveryInfo: function (deliveryNode) {
					if (this.result.DELIVERY) {
						...
						BX.cleanNode(deliveryInfoContainer),
							...
							(extraServices = this.getDeliveryExtraServices(currentDelivery)).length &&
				...
				editFadeDeliveryBlock: function () {
					...
					if (
						(this.initialized.delivery ||
							(this.editActiveDeliveryBlock(!1), BX.remove(BX.lastChild(this.deliveryBlockNode))),
							(newContent = this.getNewContainer(!0)),
						...
						var divs = BX.findChildren(BX("bx-soa-coupon"), { className: "bx-soa-section-content" });
						divs && divs.length && (BX.cleanNode(divs[0]), this.editCoupons(divs[0]));
					}
				},
				createDeliveryItem: function (item) {
					...
					item.PRICE >= 0 || void 0 !== item.DELIVERY_DISCOUNT_PRICE
						...
						: deliveryCached &&
						(deliveryCached.PRICE >= 0 || void 0 !== deliveryCached.DELIVERY_DISCOUNT_PRICE) &&
						((bHasPrice = !0),
							(costNode = BX.create("DIV", {
								props: { className: "bx-soa-pp-delivery-cost" },
								html:
						...
						(title = BX.create("DIV", {
							props: { className: "bx-soa-pp-company-smalltitle" },
							text: "N" != this.params.SHOW_DELIVERY_PARENT_NAMES ? item.NAME : item.OWN_NAME,
						})).setAttribute("lang", "ru"),
					...
					return (
						...
						checked && BX.addClass(itemNode, "bx-selected"),
						checked && this.result.LAST_ORDER_DATA.PICK_UP && (this.lastSelectedDelivery = deliveryId),
						itemNode
					);
				},
				...
				getSelectedDelivery: function () {
					...
					if (
						...
						for (i in ((deliveryId = deliveryCheckbox.value), this.result.DELIVERY))
				...
				editPickUpBlock: function (active) {
					this.pickUpBlockNode &&
						...
						((this.initialized.pickup = !1),
							active ? this.editActivePickUpBlock(!0) : this.editFadePickUpBlock(),
				...
				editActivePickUpBlock: function (activeNodeMode) {
					...
					this.initialized.pickup
						...
						: ((pickUpContent = node.querySelector(".bx-soa-section-content")) ||
							((pickUpContent = this.getNewContainer()), node.appendChild(pickUpContent)),
							...
							("Y" == this.params.SHOW_PICKUP_MAP && "Y" == this.params.SHOW_NEAREST_PICKUP) ||
				...
				editFadePickUpBlock: function () {
					var pickUpContent = this.pickUpBlockNode.querySelector(".bx-soa-section-content"),
						newContent;
					this.initialized.pickup ||
						(this.editActivePickUpBlock(!1), BX.remove(BX.lastChild(this.pickUpBlockNode))),
						(newContent = this.getNewContainer()),
					...
					selectedPickUp &&
						("Y" == this.params.SHOW_STORES_IMAGES &&
							(html +=
								'<img src="' +
								(imgSrc =
									(logotype = this.getImageSources(selectedPickUp, "IMAGE_ID")).src_1x ||
				...
				getPickUpInfoArray: function (storeIds) {
					...
					for (i = 0; i < storeIds.length; i++)
						this.result.STORE_LIST[storeIds[i]] && arr.push(this.result.STORE_LIST[storeIds[i]]);
					return arr;
				},
				...
				editPickUpMap: function (pickUpContent) {
					pickUpContent.appendChild(
						BX.create("DIV", { props: { id: "pickUpMap" }, style: { width: "100%", marginBottom: "10px" } })
					);
				},
					...
					if (this.pickUpPagination.currentPage && this.pickUpPagination.currentPage.length) {
						...
						if (
							(buyerStoreInput && (selectedStore = buyerStoreInput.value),
								...
								recommendList && recommendList.querySelector(".bx-soa-pickup-list-item.bx-selected"))
						)
							found = !0;
						else if ((selectedDelivery = this.getSelectedDelivery()) && selectedDelivery.STORE)
						...
						for (i = 0; i < this.pickUpPagination.currentPage.length; i++)
							((currentStore = this.pickUpPagination.currentPage[i]).ID != selectedStore &&
								0 != parseInt(selectedStore) &&
								found) ||
								((selectedStore = buyerStoreInput.value = currentStore.ID), (found = !0)),
				...
				createPickUpItem: function (currentStore, options) {
					...
					return (
						"Y" === this.params.SHOW_STORES_IMAGES
							...
							: ((imgClassName += " no-image"), (buttonClassName += " no-image")),
				...
				selectStore: function (event) {
					...
					if (BX.type.isString(event)) {
						if (!(storeItem = BX("store-" + event))) {
							for (i = 0; i < this.pickUpPagination.pages.length; i++)
								for (page = this.pickUpPagination.pages[i], k = 0; k < page.length; k++)
									if (page[k].ID == event) {
					...
					if (storeItem && storeInput) {
						...
						(storeItemId = storeItem.id.substr("store-".length)),
							...
							((selectedPickUp.querySelector(".bx-soa-pickup-l-item-btn .btn").innerText =
								this.params.MESS_SELECT_PICKUP),
							...
							(storeItem.querySelector(".bx-soa-pickup-l-item-btn .btn").innerText =
								this.params.MESS_SELECTED_PICKUP),
							new BX.easing({
								duration: 300,
				...
				getDeliverySortedArray: function (objDelivery) {
					...
					for (k in objDelivery)
						objDelivery.hasOwnProperty(k) &&
							("L" === this.params.SHOW_NOT_CALCULATED_DELIVERIES && objDelivery[k].CALCULATE_ERRORS
					...
					if (this.initialized.props)
						...
						if (
							((propsContent = node.querySelector(".bx-soa-section-content"))
								...
								: ((propsContent = this.getNewContainer()), node.appendChild(propsContent)),
							...
							for (i = 0; i < this.params.SHOW_MAP_FOR_DELIVERIES.length; i++)
								if (
									parseInt(selectedDelivery.ID) === parseInt(this.params.SHOW_MAP_FOR_DELIVERIES[i])
								) {
									showPropMap = !0;
									break;
								}
						this.editPropsItems(propsNode),
							...
							"Y" !== this.params.HIDE_ORDER_DESCRIPTION && this.editPropsComment(propsNode),
				...
				editFadePropsBlock: function () {
					...
					this.initialized.props ||
						(this.editActivePropsBlock(!1), BX.remove(BX.lastChild(this.propsBlockNode))),
						(newContent = this.getNewContainer()),
				...
				editPropsItems: function (propsNode) {
					if (this.result.ORDER_PROP && this.propertyCollection) {
						...
						for (
							...
							for (propsIterator = group.getIterator(); (property = propsIterator());)
				...
				getPropertyRowNode: function (property, propsItemsContainer, disabled) {
					...
					(disabled
						...
						: (BX.addClass(propsItemNode, "form-group bx-soa-customer-field"),
							property.isRequired() && (textHtml += '<span class="bx-authform-starrequired">*</span> '),
					...
					if (property.getId() in this.locations)
						if (disabled) {
							...
							for (
								...
								for (k in ((currentLocation = propRow[i] ? propRow[i].output : {}),
									(insertedLoc = BX.create("DIV", {
					...
					if (
						(BX.hasClass(lastProp, "bx-soa-loc") &&
							("search" == this.locationsTemplate
								...
								(index =
									parseInt(
										input.name.substring(input.name.indexOf("[") + 1, input.name.indexOf("]"))
									) + 1)
								: (input = lastProp.querySelectorAll("input[type=hidden]")).length &&
				...
				insertDateProperty: function (property, propsItemNode, disabled) {
					...
					if (disabled) {
						if (
							...
							for (
								values = [], dateInputs = prop.querySelectorAll("input[type=text]"), i = 0;
								i < dateInputs.length;
								i++
							)
								dateInputs[i].value && dateInputs[i].value.length && values.push(dateInputs[i].value);
				...
				insertFileProperty: function (property, propsItemNode, disabled) {
					...
					if (disabled) {
						if (
							...
							for (values = [], fileLinks = prop.querySelectorAll("a"), i = 0; i < fileLinks.length; i++)
				...
				insertStringProperty: function (property, propsItemNode, disabled) {
					...
					if (disabled) {
						if (
							...
							if (
								((values = []),
									0 == (inputs = prop.querySelectorAll("input[type=text]")).length &&
				...
				insertEnumProperty: function (property, propsItemNode, disabled) {
					...
					if (disabled) {
						if (
							...
							if (((values = []), (inputs = prop.querySelectorAll("input[type=radio]")).length))
								for (i = 0; i < inputs.length; i++)
									inputs[i].checked && values.push(inputs[i].nextSibling.nodeValue);
				...
				insertYNProperty: function (property, propsItemNode, disabled) {
					...
					if (disabled) {
						if (
							...
							for (
								values = [], inputs = prop.querySelectorAll("input[type=checkbox]"), i = 0;
								i < inputs.length;
								i += 2
							)
				...
				insertNumberProperty: function (property, propsItemNode, disabled) {
					...
					if (disabled) {
						if (
							...
							for (
								values = [], inputs = prop.querySelectorAll("input[type=text]"), i = 0;
								i < inputs.length;
								i++
							)
								inputs[i].value.length && values.push(inputs[i].value);
				...
				alterProperty: function (settings, propContainer) {
					...
					for (
						...
						((textNode.id = "soa-property-" + settings.ID),
							"Y" == settings.IS_ADDRESS && textNode.setAttribute("autocomplete", "address"),
							"Y" == settings.IS_EMAIL && textNode.setAttribute("autocomplete", "email"),
							"Y" == settings.IS_PAYER && textNode.setAttribute("autocomplete", "name"),
							"Y" == settings.IS_PHONE && textNode.setAttribute("autocomplete", "tel"),
							settings.PATTERN && settings.PATTERN.length && textNode.removeAttribute("pattern")),
					...
					for (inputs = propContainer.querySelectorAll("select"), i = 0; i < inputs.length; i++)
						BX.addClass(inputs[i], "form-control");
					for (inputs = propContainer.querySelectorAll("textarea"), i = 0; i < inputs.length; i++)
						(inputs[i].placeholder = settings.DESCRIPTION),
					...
					if ("FILE" == settings.TYPE) {
						...
						for (fileTitles = propContainer.querySelectorAll("a"), i = 0; i < fileTitles.length; i++)
					...
					for (add = propContainer.querySelectorAll("input[type=button]"), i = 0; i < add.length; i++)
						BX.addClass(add[i], "btn btn-default btn-sm"),
							("Y" == settings.MULTIPLE && i == add.length - 1) ||
							("FILE" == settings.TYPE &&
								(BX.prepend(add[i], add[i].parentNode), (add[i].style.marginRight = "10px")));
					add.length &&
				...
				isValidForm: function () {
					...
					var regionErrors = this.isValidRegionBlock(),
						...
						i;
					if (
						(regionErrors.length && ((navigated = !0), this.animateScrollTo(this.regionBlockNode, 800, 50)),
							propsErrors.length && !navigated)
					)
						if (this.activeSectionId == this.propsBlockNode.id) {
							for (
								tooltips = this.propsBlockNode.querySelectorAll("div.tooltip"), i = 0;
								i < tooltips.length;
								i++
							)
				...
				isValidRegionBlock: function () {
					...
					for (i = 0; i < regionProps.length; i++)
						(id = regionProps[i].getAttribute("data-property-id-row")),
							...
							(regionErrors = regionErrors.concat(this.isValidProperty(data, !0)));
					return regionErrors;
				},
						...
						(id = props[i].getAttribute("data-property-id-row")),
							...
							((propContainer = props[i].querySelector(".soa-property-container")) &&
								((arProperty = this.validation.properties[id]),
									...
									(propsErrors = propsErrors.concat(this.isValidProperty(data, !0)))));
					return propsErrors;
				},
				...
				getValidationData: function (arProperty, propContainer) {
					if (arProperty && propContainer) {
						...
						switch (arProperty.TYPE) {
							case "STRING":
								if (
									((data.action = "change"),
										...
										(inputs = propContainer.querySelectorAll("input[type=text]")).length)
								) {
									data.inputs = inputs;
									break;
								}
							...
							case "LOCATION":
								if (
									((data.func = BX.delegate(function (input, fieldName) {
										...
										(inputs = propContainer.querySelectorAll("input.bx-ui-sls-fake[type=text]")).length)
								) {
									(data.inputs = inputs), (data.action = "keyup");
									break;
								}
								(inputs = propContainer.querySelectorAll("div.bx-ui-slst-pool")).length &&
									(data.inputs = inputs);
								break;
							case "Y/N":
							...
							case "ENUM":
								if (
									((inputs = propContainer.querySelectorAll("input[type=radio]")).length ||
				...
				showValidationResult: function (inputs, errors) {
					if (inputs && inputs.length && errors) {
						var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
							formGroup,
							label = BX.findParent(input0, { tagName: "DIV", className: "form-group" }).querySelector(
								"label"
							),
							tooltipId,
							inputDiv,
							i;
						for (label && (tooltipId = label.getAttribute("for")), i = 0; i < inputs.length; i++)
						...
						: ("Y" === arProperty.REQUIRED &&
							...
							(arProperty.MINLENGTH &&
								...
								((re = new RegExp(arProperty.PATTERN)).test(value) ||
									errors.push(BX.message("SOA_INVALID_TEL")))),
							errors);
				},
				validateLocation: function (input, arProperty, fieldName) {
					...
					return "Y" == arProperty.MULTIPLE && "Y" !== arProperty.IS_LOCATION
						? errors
						: ("Y" != arProperty.REQUIRED ||
							(0 != value.length && value != BX.message("SOA_NOT_SPECIFIED")) ||
							errors.push(field + " " + BX.message("SOA_REQUIRED")),
							errors);
				},
				...
				validateNumber: function (input, arProperty, fieldName) {
					...
					return "Y" == arProperty.MULTIPLE
						...
						: ("Y" == arProperty.REQUIRED &&
							...
							errors.push(field + " " + BX.message("SOA_REQUIRED")),
							value.length &&
							(/[0-9]|\./.test(value) || errors.push(field + " " + BX.message("SOA_NOT_NUMERIC")),
								arProperty.MIN &&
								parseFloat(arProperty.MIN) > parseFloat(value) &&
								errors.push(
									BX.message("SOA_MIN_VALUE") + ' "' + name + '" ' + parseFloat(arProperty.MIN)
								),
								arProperty.MAX &&
								parseFloat(arProperty.MAX) < parseFloat(value) &&
								errors.push(
									BX.message("SOA_MAX_VALUE") + ' "' + name + '" ' + parseFloat(arProperty.MAX)
								),
								arProperty.STEP &&
					...
					if (
						...
						for (i = 0; i < files.length; i++)
							(file = files[i]),
								...
								(0 == fileExtension.length || "-1" == arProperty.ACCEPT.indexOf(fileExtension)) &&
								errors.push(
					...
					if (this.totalInfoBlockNode && this.result.TOTAL) {
						...
						if (
							(BX.cleanNode(this.totalInfoBlockNode),
								0 === parseFloat(total.ORDER_PRICE)
									? ((priceHtml = this.params.MESS_PRICE_FREE), (params.free = !0))
									: (priceHtml = total.ORDER_PRICE_FORMATED),
								this.options.showPriceWithoutDiscount &&
								(priceHtml +=
									'<br><span class="bx-price-old">' + total.PRICE_WITHOUT_DISCOUNT + "</span>"),
						...
						(params = {}),
							(deliveryError =
								...
								? ((deliveryValue = BX.message("SOA_NOT_CALCULATED")), (params.error = deliveryError))
								: (0 === parseFloat(total.DELIVERY_PRICE)
									? ((deliveryValue = this.params.MESS_PRICE_FREE), (params.free = !0))
							...
							this.options.showPayedFromInnerBudget
								? (this.totalInfoBlockNode.appendChild(
									this.createTotalUnit(BX.message("SOA_SUM_IT"), total.ORDER_TOTAL_PRICE_FORMATED)
								),
									this.totalInfoBlockNode.appendChild(
										this.createTotalUnit(
								...
								BX.create("DIV", {
									...
									children: [
										BX.create("A", {
											...
											events: { click: BX.proxy(this.clickOrderSaveAction, this) },
										}),
									],
								})
							),
				...
				editMobileTotalBlock: function () {
					this.result.SHOW_AUTH
						? BX.removeClass(this.mobileTotalBlockNode, "")
						: BX.addClass(this.mobileTotalBlockNode, ""),
						BX.cleanNode(this.mobileTotalBlockNode),
						this.mobileTotalBlockNode.appendChild(this.totalInfoBlockNode.cloneNode(!0)),
						BX.bind(
				...
				createTotalUnit: function (name, value, params) {
					...
					return (
						...
						(totalValue = (params = params || {}).error
							...
							: params.free
								? [BX.create("SPAN", { props: { className: "bx-soa-price-free" }, html: value })]
								: [value]),
						params.total && (className += " bx-soa-cart-total-line-total"),
						params.highlighted && (className += " bx-soa-cart-total-line-highlighted"),
						BX.create("DIV", {
				...
				basketBlockScrollCheck: function () {
					...
					for (i = 0; i < scrollableNodes.length; i++)
						(tableNode = (parentNode = scrollableNodes[i]).querySelector("table.bx-soa-item-table")),
							(parentWidth = parentNode.clientWidth),
							(tableWidth = tableNode.clientWidth || 0),
							(scrollable = scrollable || tableWidth > parentWidth)
								? ((scrollLeft = (scrollNode = BX.firstChild(parentNode)).scrollLeft),
				...
				totalBlockScrollCheck: function () {
					if (this.totalInfoBlockNode && this.totalGhostBlockNode) {
						...
						headerFixed && BX.hasClass(headerFixed, "fixed") && (offset = headerFixed.offsetHeight + 20),
				...
				totalBlockFixFont: function () {
					...
					totalNode &&
						((target = BX.lastChild(totalNode)),
							objList.push({ node: target, maxFontSize: 28, smallestValue: !1, scaleBy: target.parentNode })),
						...
						((target = BX.lastChild(buttonNode)),
							objList.push({ node: target, maxFontSize: 18, smallestValue: !1 })),

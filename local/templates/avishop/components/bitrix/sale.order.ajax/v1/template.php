...
else
{
			...
			<div class="col-sm-9 bx-soa">
				...
				<div class="pandd">
					...
					<? else: ?>
						...
						<div id="bx-soa-paysystem" data-visited="false" class="bx-soa-section bx-active">
							<div class="bx-soa-section-title-container">
								<h2 class="bx-soa-section-title col-sm-9"  style="width: 100%;">
									<span class="bx-soa-section-title-count"></span><?=$arParams['MESS_PAYMENT_BLOCK_NAME']?>
								</h2>
							</div>
							<div class="bx-soa-section-content container-fluid"></div>
				...
				<div id="bx-soa-orderSave">
					...
					<!-- начиная отсюда - кастом -->
					<div class="col-sm-12 bx-soa-pp-item-container">
						<div class="bx-soa-pp-company bx-soa-pp-company-item col-lg-4 col-sm-4 col-xs-6">
							...
							<div class="bx-soa-pp-company-smalltitle" lang="ru">Я согласен на обработку <a href="https://avivir.ru/privacy/" target="_blank"><span style="text-decoration: underline;"">персональных данных</span></a></div>
						</div>
					</div>
					...
					<div class="col-sm-12 bx-soa-pp-item-container">
						<div class="bx-soa-pp-company bx-soa-pp-company-item col-lg-4 col-sm-4 col-xs-6">
							...
							<div class="bx-soa-pp-company-smalltitle" lang="ru">Согласен с <a href="https://avivir.ru/publicoffer/" target="_blank"><span style="text-decoration: underline;"">публичной офертой</span></a></div>
						</div>
					</div>

					<div class="bx-custom-buttons">
						<a href="javascript:void(0)" id="myOrderButtonEnabled" style="margin: 10px 0; display: none;" class="btn btn-default btn-lg " data-save-button="true">
							<?=$arParams['MESS_ORDER']?>
						</a>

						<!-- Кнопка "Оформить заказ" (выключена) -->
						<a id="myOrderButtonDisabled" style="margin: 10px 0;" class="btn btn-default btn-lg" disabled>
							<?=$arParams['MESS_ORDER']?>
						</a>
						<a href="/" class="bx-custom-back-button">Вернуться в каталог</a>
					</div>

					<script>
						const confirmationParent = document.querySelector('#ID_CONFIRMATION').parentElement.parentElement;
						const ofertaParent = document.querySelector('#ID_OFERTA').parentElement.parentElement;
						const myOrderButtonEnabled = document.querySelector('#myOrderButtonEnabled');
						const myOrderButtonDisabled = document.querySelector('#myOrderButtonDisabled');

						// Функция для проверки состояния чекбоксов и переключения кнопок
						function checkCheckboxes() {
							if (confirmationParent.classList.contains('bx-selected') && ofertaParent.classList.contains('bx-selected')) {
								myOrderButtonEnabled.style.display = 'inline-block';
								myOrderButtonDisabled.style.display = 'none';
							} else {
								myOrderButtonEnabled.style.display = 'none';
								myOrderButtonDisabled.style.display = 'inline-block';
							}
						}

						// Добавляем обработчики событий для чекбоксов
						confirmationParent.addEventListener('click', () => {
							confirmationParent.classList.toggle('bx-selected');
							let checked = confirmationParent.querySelector("input[type='checkbox']").checked;
							confirmationParent.querySelector("input[type='checkbox']").checked = checked ? false : true;
							checkCheckboxes();
						});

						ofertaParent.addEventListener('click', () => {
							ofertaParent.classList.toggle('bx-selected');
							let checked = ofertaParent.querySelector("input[type='checkbox']").checked;
							ofertaParent.querySelector("input[type='checkbox']").checked = checked ? false : true;
							checkCheckboxes();
						});

						// Изначальная проверка состояния чекбоксов
						checkCheckboxes();
					</script>
					<!-- кастом заканчивается -->
				</div>
	...
	<script> 
		function checkAndSetReadOnly() {
			var elements36 = document.querySelectorAll('[data-id="36"].bx-selected');
			var elements35 = document.querySelectorAll('[data-id="35"].bx-selected');
			var inputElement = document.getElementById('soa-property-4');
			if (elements36.length > 0 || elements35.length > 0) {
		...
		var myOrderButton = document.getElementById("myOrderButtonDisabled");
		myOrderButton.addEventListener("click", function(event) {
			event.preventDefault();
			alert("Требуется получить согласие на обработку персональных данных и принятие публичной оферты.");
...
?>
<style>
	.checkout-Checkout-module-checkout #bx-soa-order-form .bx-custom-buttons, .checkout-Checkout-module-checkout .sale_order_full_table .bx-custom-buttons {
		visibility: unset;
	}
	.sdek_pvzLair {
		width: 500px;
	...
	@media(max-width: 500px) {
		#myOrderButtonEnabled.btn,
		#myOrderButtonDisabled.btn {
			width: 98%;
		}
		.checkout-Checkout-module-checkout #bx-soa-order-form .bx-custom-back-button {
			margin: 15px 0 0;
			display: flex;

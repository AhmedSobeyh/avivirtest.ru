...
if ($this->startResultCache(false, array(($arParams["CACHE_GROUPS"] === "N" ? false : $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters))) {
	...
	*/
	if ($arResult['PICTURE']) {
		$arResult['~CUSTOM_BACKGROUND'] = $arResult['PICTURE'];
	}
	...
	*/
	...
	$shortSelect = array('ID', 'IBLOCK_ID', 'IBLOCK_SECTION_ID', 'DEPTH_LEVEL');
	//$rsElement = CIBlockElement::GetList($arSort, array_merge($arFilter , $arrFilter), false, $arNavParams, $shortSelect);

	$arSort = [];

	if (strlen($arParams["ARR_ORDER"]) <= 0 || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", $arParams["ARR_ORDER"])) {
		$arSort = ["LEFT_MARGIN" => "ASC"];
	} else {
		$arSort = $GLOBALS[$arParams["ARR_ORDER"]];
		if (!is_array($arSort))
			$arSort = ["LEFT_MARGIN" => "ASC"];
	}


	if (strlen($arParams["ARR_ELEMENT_ORDER"]) <= 0 || !preg_match("/^[A-Za-z_][A-Za-z01-9_]*$/", $arParams["ARR_ELEMENT_ORDER"])) {
		$arElementSort = [];
	} else {
		$arElementSort = $GLOBALS[$arParams["ARR_ELEMENT_ORDER"]];
		if (!is_array($arElementSort))
			$arElementSort = [];
	}

	$rsElement = CIBlockSection::GetMixedList($arSort, array_merge($arFilter, $arrFilter), false, $shortSelect);
	while ($row = $rsElement->Fetch()) {
		...
		if ($row["TYPE"] == "S") {
			$arResult['TREE'][$row["ID"]] = $row;
			$arResult["SECTIONS"][] = $id;
		}

		if ($row["TYPE"] == "E") {
			// $arResult['TREE'][$row["IBLOCK_SECTION_ID"]]['ELEMENTS'][$row['ID']] = $row; //Кто это сделал - фантастика, сидел пол часа и думал почему не работает фильтр... (ниже абсолютно такая же строчка кода, только именна вот эта инициализировала массив)
	...
	if (!empty($arResult["ELEMENTS"])) {
		...
		$iterator = CIBlockElement::GetList($arElementSort, $elementFilter, false, false, $arSelect);
		...
		while ($arItem = $iterator->GetNext()) {
			...
			// Подзаголовок для фильтра
			if ($arParams["SET_SUBTITLE"] == "Y") {
				$arResult["URL_TITLE"][$arItem["DETAIL_PAGE_URL"]] = $arParams["SUBTITLE_PREFIX"] . ' ' . $arItem["NAME"] . ' ' . $arParams["SUBTITLE_POSTFIX"];
			}
	...
	foreach ($arResult["ITEMS"] as $id => $arItem) {
		...
		foreach ($arItem["PROPERTIES"] as &$prop) {
			...
			if ($prop['CODE'] == 'BRAND') {
				$arItem["BRANDS"] = \Mediahead\Helpers\Tools::getPartners($prop['VALUE']);
			}
		}

		$arResult["TREE"][$arItem['IBLOCK_SECTION_ID']]['ELEMENTS'][$id] = $arItem;
	}
	...
	if (!empty($arResult['SECTIONS'])) {
		...
		$sectionSelect = ['NAME', 'ID', 'IBLOCK_ID', 'CODE', 'DESCRIPTION', 'PICTURE', 'DETAIL_PICTURE', 'UF_SVG', 'SECTION_PAGE_URL', 'DEPTH_LEVEL'];

		$obUf = new \CUserTypeManager;
		$arResult['UF'] = $obUf->GetUserFields('IBLOCK_' . $arParams['IBLOCK_ID'] . '_SECTION');
		foreach ($arResult['UF'] as $uf) {
			$sectionSelect[] = $uf['FIELD_NAME'];
		}

		$sectionFilter = ['ID' => $arResult['SECTIONS'], 'IBLOCK_ID' => $arParams['IBLOCK_ID']];
		$rs = \CIBlockSection::getList([], $sectionFilter, false, $sectionSelect);
		while ($row = $rs->GetNext()) {
			\Mediahead\Helpers\Lang::obtainLangFields($row);
			if ($row['PICTURE']) {
				$row['PICTURE'] = \CFile::GetFileArray($row['PICTURE']);
			}
			$arResult["TREE"][$row['ID']] = array_merge($arResult["TREE"][$row['ID']], $row);
		}
	}
	...
	*/
	if ($arResult['~CUSTOM_BACKGROUND']) {
		...
		$cacheKeys[] = 'CUSTOM_BACKGROUND';
	}

	$this->setResultCacheKeys($cacheKeys);

	//apre($arResult['ITEMS'], 'list result');
...
if (isset($arResult["ID"])) {
	...
	if ($arParams["SET_SUBTITLE"] == "Y" && $arResult["URL_TITLE"]) {
		$curPage = $APPLICATION->GetCurPage();
		if (array_key_exists($curPage, $arResult["URL_TITLE"])) {
			$GLOBALS["SUBTITLE"] = $arResult["URL_TITLE"][$curPage];
		}
	}

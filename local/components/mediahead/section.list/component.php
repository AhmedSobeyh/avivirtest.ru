...
if($this->startResultCache(false, array($arrFilter, ($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()))))
{
	...
	{
		$ipropValues = new Iblock\InheritedProperty\SectionValues($arResult['SECTION']["IBLOCK_ID"], $arResult['SECTION']["ID"]);
...
if($arResult["SECTIONS_COUNT"] > 0 || isset($arResult["SECTION"]))
{
	...
	if ($arParams['SET_TITLE'] == "Y" && !empty($arResult["IPROPERTY_VALUES"]["SECTION_META_TITLE"]))
	{
		$APPLICATION->SetTitle($arResult["IPROPERTY_VALUES"]["SECTION_META_TITLE"]);
		//@todo: lang
	}

	if ($arParams['SET_TITLE'] == "Y" && !empty($arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"]))
	{
		$APPLICATION->SetTitle($arResult["IPROPERTY_VALUES"]["SECTION_PAGE_TITLE"], true);
		//@todo: lang
	}

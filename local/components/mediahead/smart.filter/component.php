...
foreach($arResult["ITEMS"] as $PID => $arItem)
{
	...
	elseif($arItem["USER_TYPE"] == "EXT_IBLOCK")
	{
		$rs = CIBlockElement::getList([],["IBLOCK_ID" => 13],false,false,["ID","NAME","PROPERTY_REL_POETRY"]);
		while($translate = $rs->Fetch())
		{
			$GLOBALS[$filter]['ID'][] = $translate["PROPERTY_REL_POETRY_VALUE"];
		}
	}
	else
	{
...
if(isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y")
{
	...
	if ($arParams["INSTANT_RELOAD"] && $arParams["COMPONENT_CONTAINER_ID"])
	{
		$arResult["INSTANT_RELOAD"] = $arParams["INSTANT_RELOAD"];
		$arResult["COMPONENT_CONTAINER_ID"] = $arParams["COMPONENT_CONTAINER_ID"];
	}
